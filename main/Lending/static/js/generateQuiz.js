$(document).ready(function () {
  ...
  function getRadioModel(questTitle, inputs, ids) {
    ...
    <div class="buttons-quiz">
        ...
        <button id = "${ids ? ids : ""}" type="button" class="quiz-button-next" style = "background-color: ${accentColor}">Далее</button>
    </div>
    </form>
    </div>
    </div>
  ...
  function getInputModel(questTitle, inputs, ids) {
    ...
    <div class="adress-form__item">
        <label for="input_${index}">${input.text}</label>
        <div class="adress-form__adress">
            <input required data-question="1" type="text" id="input_${index}" name="${questTitle}" placeholder="${input.placeholder}">
    ...
    <div class="quiz__slide swiper-slide">
        <div class="quiz__content">
            <div class="quiz__caption"><b>${questTitle}</b></div>
            <form class="adress-form" action="#">
                <div class="adress-form__items">${inputFields}</div>
                <div class="buttons-quiz">
                    <button type="button" class="quiz-button-prev"></button>
  ...
  $(document).on("input", ".quiz__slide input[type='text']", function () {
    ...
    const button = currentFrame.find(".quiz-button-next");
    const requiredInputs = currentFrame.find("input[required]");
