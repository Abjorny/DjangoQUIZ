$(document).ready(function () {
  ...
  function closePopup(e) {
    if (
      ...
      e.target.classList.contains("popup__wrapper")
    ) {
  ...
  popupButton.forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      popups.forEach((popup) => {
        if (
          popup.dataset.popup == data ||
          popup.dataset.popup ==
            e.target.closest(".header__social, .header__tel-text").dataset
              .popupOpen
        )
          openPopup(popup);
      });
  ...
  const getCurrentSliderData = () =>
    window.innerWidth <= 768 ? hardestSliderDataMobile : hardestSliderData;
  ...
  function updateHardestSlider(index) {
    ...
    const currentImageIndex = hardestCurrentImageIndex;
  ...
  hardestThumbnails.forEach((thumb, index) => {
    thumb.addEventListener("click", () => {
      ...
      if (window.innerWidth <= 11768)
        hardestDescription.innerHTML = currentSlider.description;
    });
  });
  ...
  function showPrevImage() {
    ...
    if (hardestCurrentImageIndex === 0) {
      hardestCurrentSliderIndex =
        (hardestCurrentSliderIndex - 1 + sliderData.length) % sliderData.length;
      ...
      if (window.innerWidth <= 11768)
        hardestDescription.innerHTML = newSlider.description;
    } else {
  ...
  function showNextImage() {
    ...
    if (hardestCurrentImageIndex === currentSlider.thumbnails.length - 1) {
      hardestCurrentSliderIndex =
        (hardestCurrentSliderIndex + 1) % sliderData.length;
      const newSlider = sliderData[hardestCurrentSliderIndex];
      hardestCurrentImageIndex = 0;
      hardestMainImage.src = newSlider.thumbnails[hardestCurrentImageIndex];
      hardestThumbnails.forEach((thumb, i) => {
